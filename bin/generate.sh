...
if [ "$1" = "" ] || [ ! -f "$1" ]; then
  echo
  echo "Please provide a Drupsible profile, previously generated by bin/configure.sh."
  exit 1
else
  source "$1"
fi
...
sed -i.bak "s/private-key-file:.*/private-key-file: '${KEY_FILENAME}'/g" vagrant.yml
...
do
  ...
  if [ ! -f "ansible/inventory/${APP_NAME}${ENV}" ]; then
    ...
    sed -i.bak "s/app_name/${APP_NAME}/g" "ansible/inventory/${APP_NAME}${ENV}"
    # Assign hostname
    sed -i.bak "s/app_webhost=.*/app_webhost=${HOSTNAME}/g" "ansible/inventory/${APP_NAME}${ENV}"
  else
    echo
...
do
  ...
  if [ "$ENV" != "-local" ]; then
    sed -i.bak "s/app_user:.*/app_user: ${APP_NAME}/g" all.yml
  else
    sed -i.bak "s/app_user:.*/app_user: vagrant/g" all.yml
  fi
  sed -i.bak "s/app_drupal_version:.*/app_drupal_version: '${DRUPAL_VERSION}'/g" all.yml
  if [ "$MULTILINGUAL" == "yes" ]; then
    sed -i.bak "s|app_i18n_enabled:.*$|app_i18n_enabled: yes|g" all.yml
    if [ "$LANGUAGES" != "" ]; then
      sed -i.bak "s|app_languages:.*$|app_languages: [ ${LANGUAGES} ]|g" all.yml
    fi
  else
    sed -i.bak "s|app_i18n_enabled:.*$|app_i18n_enabled: no|g" all.yml
  fi
  ...
  if [ "$USE_INSTALL_PROFILE" == "yes" ]; then
    sed -i.bak "s/deploy_install_profile_enabled:.*$/deploy_install_profile_enabled: 'yes'/g" deploy.yml
    if [ "$D_O_INSTALL_PROFILE" != "" ]; then
      sed -i.bak "s/deploy_d_o_install_profile:.*$/deploy_d_o_install_profile: '${D_O_INSTALL_PROFILE}'/g" deploy.yml
      sed -i.bak "s/deploy_custom_install_profile:.*$/deploy_custom_install_profile: ''/g" deploy.yml
    elif [ "$CUSTOM_INSTALL_PROFILE" != "" ]; then
      sed -i.bak "s/deploy_custom_install_profile:.*$/deploy_custom_install_profile: '${CUSTOM_INSTALL_PROFILE}'/g" deploy.yml
      sed -i.bak "s/deploy_d_o_install_profile:.*$/deploy_d_o_install_profile: ''/g" deploy.yml
    else
      sed -i.bak "s/deploy_d_o_install_profile:.*$/deploy_d_o_install_profile: standard/g" deploy.yml
    fi
    if [ "$USE_DRUSH_MAKE" == "yes" ]; then
      sed -i.bak "s|deploy_drush_make_enabled:.*$|deploy_drush_make_enabled: 'yes'|g" deploy.yml
      sed -i.bak "s|deploy_composer_enabled:.*$|deploy_composer_enabled: 'no'|g" deploy.yml
      if [ "$DRUSH_MAKEFILE" != "" ]; then
        sed -i.bak "s|deploy_drush_makefile:.*$|deploy_drush_makefile: '${DRUSH_MAKEFILE}'|g" deploy.yml
      fi
    elif [ "$USE_COMPOSER" == "yes" ]; then
      sed -i.bak "s|deploy_composer_enabled:.*$|deploy_composer_enabled: 'yes'|g" deploy.yml
      sed -i.bak "s|deploy_drush_make_enabled:.*$|deploy_drush_make_enabled: 'no'|g" deploy.yml
    else
      sed -i.bak "s|deploy_drush_make_enabled:.*$|deploy_drush_make_enabled: 'no'|g" deploy.yml
      sed -i.bak "s|deploy_composer_enabled:.*$|deploy_composer_enabled: 'no'|g" deploy.yml
    fi
    ...
    if [ ! "$CODEBASE_TARBALL" == "" ]; then
      sed -i.bak "s|deploy_codebase_tarball_filename:.*$|deploy_codebase_tarball_filename: '${CODEBASE_TARBALL}'|g" deploy.yml
      sed -i.bak "s|deploy_codebase_import_enabled:.*$|deploy_codebase_import_enabled: yes|g" deploy.yml
    else
      sed -i.bak "s|deploy_codebase_import_enabled:.*$|deploy_codebase_import_enabled: no|g" deploy.yml
    fi
    sed -i.bak "s/deploy_install_profile_enabled:.*$/deploy_install_profile_enabled: 'no'/g" deploy.yml
  fi
  if [ "$USE_UPSTREAM_SITE" == "yes" ]; then
    sed -i.bak "s|deploy_db_sync_enabled:.*$|deploy_db_sync_enabled: '${SYNC_DB}'|g" deploy.yml
    sed -i.bak "s|deploy_files_sync_enabled:.*$|deploy_files_sync_enabled: '${SYNC_FILES}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_remote_host:.*$|deploy_upstream_remote_host: '${REMOTE_UPSTREAM_HOST}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_remote_port:.*$|deploy_upstream_remote_port: '${REMOTE_UPSTREAM_PORT}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_remote_user:.*$|deploy_upstream_remote_user: '${REMOTE_UPSTREAM_USER}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_docroot:.*$|deploy_upstream_docroot: '${REMOTE_UPSTREAM_DOCROOT}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_files_path:.*$|deploy_upstream_files_path: '${REMOTE_UPSTREAM_FILES_PATH}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_proxy_credentials:.*$|deploy_upstream_proxy_credentials: '${REMOTE_UPSTREAM_PROXY_CREDENTIALS}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_proxy_port:.*$|deploy_upstream_proxy_port: '${REMOTE_UPSTREAM_PROXY_PORT}'|g" deploy.yml
    sed -i.bak "s|deploy_upstream_ssh_options:.*$|deploy_upstream_ssh_options: '${REMOTE_UPSTREAM_SSH_OPTIONS}'|g" deploy.yml
  fi
  if [ "$USE_SITE_INSTALL" == "yes" ]; then
    sed -i.bak "s|deploy_site_install_enabled:.*$|deploy_site_install_enabled: yes|g" deploy.yml
  else
    if [ ! "$DBDUMP" == "" ]; then
      sed -i.bak "s|deploy_db_dump_filename:.*$|deploy_db_dump_filename: '${DBDUMP}'|g" deploy.yml
      sed -i.bak "s|deploy_db_import_enabled:.*$|deploy_db_import_enabled: yes|g" deploy.yml
    else
      sed -i.bak "s|deploy_db_import_enabled:.*$|deploy_db_import_enabled: no|g" deploy.yml
    fi
    if [ ! "$FILES_TARBALL" == "" ]; then
      sed -i.bak "s|deploy_files_tarball_filename:.*$|deploy_files_tarball_filename: '${FILES_TARBALL}'|g" deploy.yml
      sed -i.bak "s|deploy_files_import_enabled:.*$|deploy_files_import_enabled: yes|g" deploy.yml
    else
      sed -i.bak "s|deploy_files_import_enabled:.*$|deploy_files_import_enabled: no|g" deploy.yml
    fi
  fi
  # Git config
  sed -i.bak "s/deploy_git_repo_protocol:.*$/deploy_git_repo_protocol: \"${GIT_PROTOCOL}\"/g" deploy.yml
  sed -i.bak "s/deploy_git_repo_server:.*$/deploy_git_repo_server: \"${GIT_SERVER}\"/g" deploy.yml
...
echo "                                                =========="
echo "Vagrant will run the drupsible VM by default. You can edit vagrant.yml to change"
echo "the VM box and other configuration values like the amount of RAM."
