...
tar czvf "$BACKUP_DIR/$BACKUP_FILENAME" --exclude "*.default" --exclude "*default.profile" --exclude "*/.git" --exclude "*.gz" --exclude "*.zip" --exclude "lookup_plugins" --exclude "*example.com.yml" --exclude "README.*" "ansible" >/dev/null
if [ "$?" == 0 ]; then
